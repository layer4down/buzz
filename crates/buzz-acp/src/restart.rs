...
fn driver_main(
    ...
    // Phase 2 — single respawn.
    let spawned = if label != "-" {
        kickstart_launchd(uid, label)
    } else if script != "-" {
        exec_start_script(script, workdir)
    } else {
    ...
    std::thread::sleep(RESPAWN_VERIFY_WAIT);
...
fn kickstart_launchd(uid: &str, label: &str) -> Result<u32, String> {
    ...
    // launchctl print reports the job's pid; parse the first "pid = N" hit.
    ...
    text.lines()
        ...
        .ok_or_else(|| "no pid line in launchctl print output".to_string())
}

fn exec_start_script(script: &str, workdir: &Path) -> Result<u32, String> {
    ...
    // Seats write acp.pid beside their logs (start.sh contract).
    fs::read_to_string(workdir.join("acp.pid"))
        .ok()
        .and_then(|c| c.trim().parse::<u32>().ok())
        .ok_or_else(|| "no acp.pid after start script".to_string())
}
